...
class Console : ClassBase
{
    ...
    {
        Array classFoldPathArray;
        classFoldPathArray = this.ClassFoldPathArray(path);
        return false;
    }

    private Array ClassFoldPathArray(String path)
    {
        Text textPath;
        textPath = this.TextCreate(path);

        long kaa;
        kaa = this.StorageInfra.EntryPathNameCombine(textPath, this.TLess);
        ...
        textPath.Range.Count = kaa;

        String foldPath;
        foldPath = this.StringCreate(textPath);

        String text;
        text = this.StorageInfra.TextRead(path);

        Array arrayA;
        arrayA = this.TextLimit(this.TA(text), this.TB(this.TextInfra.NewLine));
        ...
        {
            Text ka;
            ka = arrayA.GetAt(i) as Text;

            bool b;
            b = this.StorageInfra.PathRelate(ka, this.TLess);

            if (b)
            {
                String relatePath;
                relatePath = this.StringCreate(ka);

                String kk;
                kk = this.AddClear().Add(foldPath).Add(this.TextInfra.PathCombine).Add(relatePath).AddResult();

                kk = this.CleanPath(kk);
            ...
            {
                String absolutePath;
                absolutePath = this.StringCreate(ka);

                array.SetAt(i, absolutePath);
            }
